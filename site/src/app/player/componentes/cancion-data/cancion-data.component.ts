...
export class CancionDataComponent implements OnInit {
  ...
  getCancion(): void {
    const entryParam: string = this.route.snapshot.paramMap.get("cancionId") ?? "";
    if (entryParam !== "") {
  ...
  getValoracion(): void {
    this.cancionService.getValoracionCancionById(this.cancion!.id!.toString(), this.usuarioId!.toString()).subscribe({
